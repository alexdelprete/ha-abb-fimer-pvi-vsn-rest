...
**Entity Naming Pattern (has_entity_name=False):**

- **Device Identifier**: `{DOMAIN}_{device_type}_{serial_compact}`
...
- **Entity ID** (auto-generated by HA):
  ...
  - Example: `sensor.abb_fimer_pvi_vsn_rest_inverter_0779093g823112_ac_power`
  - **Changed in v1.0.0-beta.22**: Added domain prefix for namespace isolation

- **Unique ID**: Aligned with entity ID format (v1.0.0-beta.22+)
  - Format: `{domain}_{device_type}_{serial_compact}_{point_name}`
  - Example: `abb_fimer_pvi_vsn_rest_inverter_0779093g823112_watts`
  - **Changed in v1.0.0-beta.22**: Added domain prefix, aligned with entity_id
...
**Decision**: Use `has_entity_name=False` pattern with `suggested_object_id` to control entity IDs.

**Rationale**:

- Allows both custom entity IDs (via `suggested_object_id`) and friendly display names (via `_attr_name`)
- Clean device cards with meaningful names
- Predictable entity ID format: `sensor.{domain}_{device_type}_{serial_compact}_{point_name}`
...
- `suggested_object_id`: Controls entity_id (e.g., "abb_fimer_pvi_vsn_rest_inverter_0779093g823112_ac_power")
- Entity ID: `sensor.{suggested_object_id}`
- Result: Predictable IDs with friendly names

**Note**: `has_entity_name=True` does NOT work with `suggested_object_id` - HA ignores it and generates entity_id from device_name + entity_name instead.
...
```python
# sensor.py - Entity naming configuration
self._attr_has_entity_name = False  # Required for suggested_object_id to work
...
# Suggested object ID (controls entity_id when has_entity_name=False)
self._attr_suggested_object_id = f"{DOMAIN}_{device_type_simple}_{device_sn_compact}_{point_name}"

# Entity name (friendly display)
...
- ✅ Use `has_entity_name=False` with `suggested_object_id` for predictable entity IDs
