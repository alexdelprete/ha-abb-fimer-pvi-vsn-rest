...
This release fixes excessive logging during device offline periods, such as when the inverter is offline at night (no
solar power). Previously, the integration would log
...
**Problem**: When the inverter is offline (e.g., nighttime when there's no solar power), the integration logged WARNING
and ERROR messages every poll interval (default: 60
seconds).
