...
This is the **first beta release** of the ABB/FIMER PVI VSN REST integration for Home Assistant. This new integration provides native REST API support for ABB/FIMER/Power-One
inverters via VSN300 and VSN700 dataloggers.
...
1. Add integration via UI: Settings → Devices & Services → Add Integration → ABB/FIMER PVI VSN REST
...
1. **Note:** Entity IDs may differ, so you'll need to update automations/dashboards
...
1. **Configuration** - Test config flow completes successfully
1. **Data Collection** - Confirm sensors are created and updating
1. **VSN Model Detection** - Check that correct VSN model is detected

### Advanced Testing

1. **Scan Interval** - Test different polling frequencies
1. **Error Handling** - Test behavior when VSN is unreachable
1. **Reconfiguration** - Test changing host/credentials via reconfigure
1. **Multiple Inverters** - If applicable, verify all inverters are discovered
...
1. REST client fetches data from VSN endpoints
1. Data normalized using VSN→SunSpec mapping
1. Sensors update with new values
1. Home Assistant UI reflects changes
