...
2. **device_type attribute** - Still showing "unknown" for datalogger entities after beta.27 fix

Both issues were investigated and root causes identified. The fixes ensure proper data flow through the discovery → client → normalizer → sensor pipeline.

---

## Bug Fixes

### 🐛 sys_time - Raw Number Instead of Date/Time
...
**Solution**:
Update mapping file to set device_class to "timestamp".

**Code Change** ([vsn-sunspec-point-mapping.json:544](d:\\OSILifeDrive\\Dev\\HASS\\ha-abb-fimer-pvi-vsn-rest\\custom_components\\abb_fimer_pvi_vsn_rest\\data\\vsn-sunspec-point-mapping.json#L544)):
```json
"HA Device Class": "timestamp",
```

**Result**:
- ✅ Normalizer sets device_class from mapping file
- ✅ Sensor conversion code executes (line 418 condition passes)
- ✅ Raw value (816350692) converted using Aurora epoch offset (946684800)
- ✅ Displays as formatted timestamp with timezone: "2025-11-13 10:15:32+01:00"

---

### 🔧 device_type - Still "unknown" After Beta.27 Fix
...
**Root Cause**:
Beta.27 created the synthetic datalogger in discovery.py:
```python
# Beta.27 fix - discovery.py lines 291-322
datalogger_device = DiscoveredDevice(
    device_id=logger_id,
    device_type="datalogger",  # ✓ Correct here
    ...
)
...
**Data Flow Problem**:
1. Discovery creates synthetic datalogger → `device_type="datalogger"` ✓
2. Client calls `get_livedata()` → returns raw data from VSN API
3. Livedata **doesn't contain device_type for datalogger** (it's not in that endpoint)
4. Normalizer processes livedata → sets `device_type="unknown"` for missing field
5. Sensor reads from normalized data → gets `"unknown"` ✗

**The Issue**: Discovery result and normalized data are separate data structures that don't communicate.

**Solution**:
Inject device_type from discovered_devices into raw livedata before normalization.

**Implementation**:

**1. Update client.__init__ to accept discovered_devices** ([client.py:35,56](d:\\OSILifeDrive\\Dev\\HASS\\ha-abb-fimer-pvi-vsn-rest\\custom_components\\abb_fimer_pvi_vsn_rest\\abb_fimer_vsn_rest_client\\client.py#L35)):
```python
def __init__(
    self,
...
**2. Inject device_type in get_normalized_data()** ([client.py:185-200](d:\\OSILifeDrive\\Dev\\HASS\\ha-abb-fimer-pvi-vsn-rest\\custom_components\\abb_fimer_pvi_vsn_rest\\abb_fimer_vsn_rest_client\\client.py#L185-L200)):
```python
...
**3. Pass discovered_devices to client** ([__init__.py:116](d:\\OSILifeDrive\\Dev\\HASS\\ha-abb-fimer-pvi-vsn-rest\\custom_components\\abb_fimer_pvi_vsn_rest\\__init__.py#L116)):
```python
...
**Result**:
- ✅ Client receives discovered_devices list during initialization
- ✅ After fetching raw livedata, device_type injected from discovery
- ✅ Normalizer processes enriched raw data with device_type
- ✅ Normalized data contains correct device_type
- ✅ Sensor reads `device_type: "datalogger"` from normalized data
- ✅ Entity attributes show proper device type

**New Data Flow** (Fixed):
1. Discovery creates synthetic datalogger → `device_type="datalogger"` ✓
2. Client receives discovered_devices during initialization ✓
3. Client calls `get_livedata()` → raw data from API
4. **Client injects device_type into raw data from discovered_devices** ✓
5. Normalizer processes enriched raw data → `device_type="datalogger"` ✓
6. Sensor reads from normalized data → gets `"datalogger"` ✓
...
| File | Lines | Change |
|------|-------|--------|
| vsn-sunspec-point-mapping.json | 544 | Set device_class to "timestamp" for sys_time |
| client.py | 35 | Added discovered_devices parameter to __init__ |
| client.py | 56 | Store discovered_devices list |
| client.py | 185-200 | Inject device_type from discovery before normalization |
| __init__.py | 116 | Pass discovered_devices to client initialization |
| manifest.json | 13 | Version bump to 1.0.0-beta.28 |
| const.py | 4 | Version bump to 1.0.0-beta.28 |
