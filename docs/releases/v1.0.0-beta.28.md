...
Both issues were investigated and root causes identified. The fixes ensure proper data flow through the discovery →
client → normalizer → sensor pipeline.
...
1. Client calls `get_livedata()` → returns raw data from VSN API
1. Livedata **doesn't contain device_type for datalogger** (it's not in that endpoint)
1. Normalizer processes livedata → sets `device_type="unknown"` for missing field
1. Sensor reads from normalized data → gets `"unknown"` ✗
...
1. Discovery creates synthetic datalogger → `device_type="datalogger"` ✓
1. Client receives discovered_devices during initialization ✓
1. Client calls `get_livedata()` → raw data from API
1. **Client injects device_type into raw data from discovered_devices** ✓
1. Normalizer processes enriched raw data → `device_type="datalogger"` ✓
1. Sensor reads from normalized data → gets `"datalogger"` ✓
