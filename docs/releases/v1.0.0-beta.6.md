...
**Root Cause:** In the sensor initialization debug log ([sensor.py:182](custom_components/abb_fimer_pvi_vsn_rest/sensor.py#L182)), the code was directly accessing
`self._attr_native_unit_of_measurement` without using `getattr()`. This attribute might not be set for all sensors (especially string sensors), causing an AttributeError during
sensor creation.

**Fix:** Changed to use `getattr()` with a default value for consistent handling:
...
**Fix:** Removed redundant `else` clause after `return` statement:
...
1. Replace `custom_components/abb_fimer_pvi_vsn_rest` directory
1. Restart Home Assistant
