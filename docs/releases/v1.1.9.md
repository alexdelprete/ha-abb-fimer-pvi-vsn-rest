...
1. **SysTime sensor unavailable with precision error** - ValueError about numeric vs string value
...
**Problem**: Four energy sensors were displaying in Wh instead of kWh after v1.1.8:
...
- The normalizer correctly converted values: `7262065 Wh / 1000 = 7262.065 kWh` ✓
...
- Normalizer converts: `7262065 / 1000 = 7262.065` ✓
- But mapping says: `unit="Wh"` ✗
- Display shows: `7262.065 Wh` ✗ (wrong unit label!)

**Fix**: Changed generator script to set `unit="kWh"` for these 4 sensors:
...
- Energia - Anno Scorso: `7.262 kWh` ✓
- Energia - Oggi: `8.013 kWh` ✓
- Energia - Ultimi 7 Giorni: `19.303 kWh` ✓
- Energia - Ultimi 30 Giorni: `265.673 kWh` ✓

______________________________________________________________________

### 🐛 Bug #2: SysTime Sensor Precision Error (Critical)

**Problem**: SysTime sensor was unavailable with error about numeric vs string value:

```text
ValueError: Sensor sensor.power_one_inverter_pvi_10_0_outd_076543_3f71_2345_system_time
...
- v1.1.8 removed `device_class="timestamp"` from SysTime (to prevent "2 hours ago" display) ✓
...
- Having `precision=0` makes HA think it's numeric → ValueError when it returns a string
...
"SysTime": {
    ...
    "unit": "",
    "state_class": "",
    "precision": 0,  # ✗ Makes HA think it's numeric!
},
...
1. BUT the code checks SUNSPEC_TO_HA_METADATA first (takes priority), so we also had to remove the manual `precision: 0` entry
...
**Mapping Files** (regenerated): 2. All 3 mapping JSON copies (output/, docs/, client data/) 3. Excel mapping file with corrected metadata

**Version Files**: 4. `custom_components/.../const.py` (version 1.1.9) 5. `custom_components/.../manifest.json` (version 1.1.9)
...
1. **SysTime**: Removed `device_class="timestamp"` to fix "2 hours ago" display, but forgot to also remove `precision=0` which causes the same numeric sensor classification issue
   as state sensors
...
- ✅ DayWH, WeekWH, MonthWH, YearWH display in kWh (not Wh)
- ✅ SysTime loads without errors and displays formatted string
...
Display shows:     7.262 kWh ✓ (was 7262.065 Wh in v1.1.8)
```text
...
Mapping has:        no precision field ✓
Display shows:      "2025-11-17 14:30:13" ✓ (not "unavailable")
```text
