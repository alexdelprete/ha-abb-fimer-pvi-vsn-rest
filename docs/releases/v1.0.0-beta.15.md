...
This release focuses on improving the quality and consistency of point descriptions and display names across all 258
mapped data points. The changes make entity names more
...
**Problem:** Temperature sensors showed "degC" instead of "°C" and had 2 decimal precision.

**Solution:** Normalized temperature units to "°C" and changed display precision to 1 decimal for better readability.
...
- Unit: "degC" → "°C"
- Precision: 2 decimals → 1 decimal
- Display: "25.67°C" → "25.7°C"
...
- "Export energy counter" → "Export energy"
- "Import energy counter" → "Import energy"
- "Consumed energy counter" → "Consumed energy"
- "Produced energy counter" → "Produced energy"
...
- "PV production energy last month" → "PV production energy - Month"
...
- **cosPhi**: "Power Factor" (removed "Cos φ" Greek symbol)
- **mfg_serial_number**: "Serial Number" → "Manufacturing Week/Year" (restored "Manufacturing" prefix for clarity)
- **mfg_week_year**: "Week/Year" → "Manufacturing Week/Year"
...
- DCA_1: "DC current for string 1" → "DC Current (String 1)" → Display: "DC Current #1"
- DCW_2: "DC power for string 2" → "DC Power (String 2)" → Display: "DC Power #2"

**Energy Counters:**

- Ein1: "Total DC energy from MPPT string 1" → "Total DC Energy (String 1)" → Display: "String 1 Energy"

**String Power:**

- Pin: "Combined DC input power from all strings" → "Total DC Input Power"
- Pin1: "DC input power from MPPT string 1" → "DC Input Power (String 1)" → Display: "String 1 Power"
...
- **logger_sn**: "Logger serial number" → Display: "Logger Serial Number"
...
1. ✓ All 258 points load correctly
1. ✓ Temperature units show "°C" instead of "degC"
1. ✓ Temperature sensors display with 1 decimal precision
1. ✓ Timestamp sensor (sys_time) converts to datetime with HA timezone
1. ✓ All energy point time periods use consistent suffixes (- Total, - Lifetime, - Week, - Month, - Year, - Today)
1. ✓ No "counter" word in energy descriptions
1. ✓ Phase naming is consistent across all points (A/B/C/N format)
1. ✓ No "measurement" word in descriptions
1. ✓ String references use concise "(String #)" format
1. ✓ All improved point descriptions (GlobState, PowerPeakAbs, sys_load, store_size, PF, etc.)
1. ✓ Display names are clean and concise (cosPhi, mfg_week_year)
1. ✓ Excel and JSON mappings are in sync
1. ✓ Generator script works correctly
1. ✓ No ruff errors or warnings
...
- Temperature units changed from "degC" to "°C"
...
- Temperature sensors show "°C" with 1 decimal precision
