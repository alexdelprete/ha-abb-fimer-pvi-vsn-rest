...
This release significantly improves the user experience with enhanced entity display names and fast offline detection.
Entity names are now human-readable with proper descriptions
instead of cryptic abbreviations. The integration also detects offline devices much faster (milliseconds vs 10+
seconds), providing better responsiveness during nighttime
operation.
...
**Solution:** Implemented comprehensive mapping system with 20-column Excel/JSON mapping that includes HA-specific
metadata and intelligent description priority system.
...
1. **VSN Feeds Titles** - Device-specific labels from VSN JSON
1. **Enhanced Generation** - Intelligent transformation from point names
1. **SunSpec Label** - Fallback abbreviations

**Special Handling:**

- M1 Common Model points now show user-friendly labels ("Manufacturer", "Model") instead of generic boilerplate
...
**Problem:** During nighttime (when inverter is off), the integration waited for full HTTP timeout (~10 seconds) on
every failed connection attempt, causing:

- Slow failure detection
- Poor user experience with delayed unavailability status
- Unnecessary network overhead

**Solution:** Added socket connectivity check before all VSN device HTTP requests. Socket check fails fast
(milliseconds) when device is offline/unreachable, avoiding expensive
...
**Real-World Impact:** When inverter powers down at night:
...
1. ABB Proprietary points (`HousePInverter`, `PacStandAlone`) had no units
1. Unit conversion issues with kW/kWh values
...
1. Label
1. Description
1. **HA Display Name** ← NEW - What users see in HA
1. SunSpec Model
1. Category
1. Units
1. **HA Unit of Measurement** ← NEW
1. State Class
1. **HA State Class** ← NEW
1. Device Class
1. **HA Device Class** ← NEW
1. Entity Category
1. Available in Modbus
1. **Data Source** ← NEW - For debugging
