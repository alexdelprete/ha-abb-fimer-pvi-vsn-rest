...
**Focus**: Fix 3 critical bugs introduced in v1.1.7 (state sensors unavailable, timestamp display issue, energy sensor
values wrong by 1000x)
...
```text
ValueError: Sensor sensor.power_one_inverter_pvi_10_0_outd_076543_3f71_2345_global_status
...
1. normalizer checked `if mapping.units == "Wh"` → false for those sensors
