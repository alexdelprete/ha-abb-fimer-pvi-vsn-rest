...
2. **SysTime displaying "2 hours ago"** instead of formatted date/time
3. **Energy sensors showing wrong values** (either in Wh instead of kWh, or Wh values displayed as kWh = 1000x error)
...
**Problem**:
State sensors (GlobalSt, DcSt1, DcSt2, InverterSt, AlarmState, AlarmSt) were unavailable with error:
...
2. **Generator**: Removed `"precision": ""` entries from all 6 state sensor metadata
3. **sensor.py**: Added defensive check: `if precision is not None and precision != ""`
...
### 🐛 Bug #2: SysTime Showing "2 hours ago" (Annoyance)

**Problem**:
SysTime sensor had `device_class="timestamp"` which caused Home Assistant to display relative time ("2 hours ago") instead of the formatted date/time string from the inverter.
...
**Fix**:
**Generator**: Changed SysTime metadata:
```python
"SysTime": {
...
**Problem**:
Energy sensors were displaying incorrect values:
...
**Root Cause**:
normalizer.py conversion logic was unit-based:
```python
# BROKEN v1.1.7 logic:
...
2. normalizer checked `if mapping.units == "Wh"` → false for those sensors
3. Sensors with `mapping.units == "kWh"` skipped conversion
4. VSN API always returns Wh values
5. Result: Wh values displayed as kWh = 1000x error

**Example**:
- API returns: `108892416` (Wh)
- Mapping says: `unit="kWh"`
- Normalizer: Skips conversion (units != "Wh")
- Display: `108,892.416 kWh` ✗ (should be `108.892 kWh`)

**Fix**:
Changed conversion logic from unit-based to device_class-based:
...
- VSN API always returns Wh → always convert to kWh → always display kWh
- No exceptions, no edge cases

**Result**: ALL energy sensors now display correctly in kWh with proper values:
- Energia - Anno Scorso: `7,261.292 kWh` ✓
- Energia - Oggi: `7.240 kWh` ✓
- Energia - Ultimi 7 Giorni: `18.530 kWh` ✓
- Energia - Ultimi 30 Giorni: `264.900 kWh` ✓
- Energia AC - Total: `108.892 kWh` ✓ (was 108,892.416 kWh)
...
**Runtime Code**:
2. `custom_components/.../sensor.py` (1 change)
...
4. All 3 mapping JSON copies (output/, docs/, client data/)
5. Excel mapping file with corrected metadata

**Version Files**:
6. `custom_components/.../const.py` (version 1.1.8)
7. `custom_components/.../manifest.json` (version 1.1.8)
...
2. **SysTime**: Added `device_class="timestamp"` thinking it would improve timestamp handling, but this causes relative time display and requires datetime objects

3. **Energy Sensors**: Set some sensors to `unit="kWh"` in mapping to match display, but normalizer only converted if `mapping.units == "Wh"`, creating a mismatch
...
**Verified Fixes**:
- ✅ State sensors load without errors and display text values
- ✅ SysTime displays formatted date/time string ("2025-11-17 13:52:14")
- ✅ ALL energy sensors display in kWh with correct values
- ✅ No errors in Home Assistant logs
- ✅ All 258 sensors working correctly

**Energy Sensor Verification**:
```
API returns:       7240 Wh
Normalizer converts: 7240 / 1000 = 7.240 kWh
Display shows:     7.240 kWh ✓

API returns:       108892416 Wh
Normalizer converts: 108892416 / 1000 = 108892.416 kWh
Display shows:     108.892 kWh ✓ (was 108,892.416 kWh in v1.1.7)
```
