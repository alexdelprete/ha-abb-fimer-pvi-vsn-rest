...
This release adds support for REACT2 inverters and improves the VSN authentication detection logic. REACT2 devices use VSN700 dataloggers with preemptive Basic authentication but
don't send WWW-Authenticate headers in the same way as standard VSN700 devices. The integration now handles this correctly.

______________________________________________________________________

## What's New

### ✨ Feature: REACT2 Inverter Support
...
- ✅ REACT2 inverters now fully supported
- ✅ No configuration changes needed - automatic detection
- ✅ Works with existing VSN700 credentials
...
### 🔧 Improvement: Simplified Authentication Detection
...
### 📝 Improvement: Authentication Debug Logging
...
### 📚 Improvement: Complete Translation Quality
...
"battery": "Battery"                 ❌ (English)
"model": "Model identifier"          ❌ (Mixed)
```text

**After**:

```text
"connection": "Verbindung"           ✅ (German)
"battery": "Batterie"                ✅ (German)
"model": "Modellkennung"             ✅ (German)
```text

**Languages Fixed**:

- 🇫🇷 French (`fr`) - 251 sensors
- 🇪🇸 Spanish (`es`) - 251 sensors
- 🇵🇹 Portuguese (`pt`) - 251 sensors
- 🇩🇪 German (`de`) - 251 sensors
- 🇸🇪 Swedish (`sv`) - 251 sensors
- 🇳🇴 Norwegian (`nb`) - 251 sensors
- 🇫🇮 Finnish (`fi`) - 251 sensors
- 🇪🇪 Estonian (`et`) - 251 sensors
- 🇮🇹 Italian (`it`) - 251 sensors

**Total**: 2,259 sensor translations (251 × 9 languages) now complete and professional
...
1. Check for `X-Digest` header:
   - **Found** → VSN300 (return immediately)
   - **Not found** → Try preemptive Basic auth
     - **Success (200/204)** → VSN700
     - **Failure** → Error (unsupported device)
...
1. **Enable debug logging** (optional, for verification):
...
- **Inverters**: ABB, FIMER, Power-One PVI series, **REACT2 series** ✨ NEW
...
- fix: Complete and improve all 9 language translations ([e3001df](https://github.com/alexdelprete/ha-abb-fimer-pvi-vsn-rest/commit/e3001df))
  - Fixed 2,259 sensor translations (251 × 9 languages)
