...
### 🐛 Bug: Energy Sensors Still Showing Wh Instead of kWh (Critical)

**Problem**:
Despite v1.1.9 changes:
1. Generator set `unit="kWh"` in mapping ✓
2. Normalizer converted values Wh→kWh ✓
...
if mapping.device_class == "energy" and point_value is not None:
    if isinstance(point_value, (int, float)) and point_value != 0:
        ...
        if mapping.units != "kWh":
            mapping.units = "kWh"  # ← Trying to modify dataclass attribute

normalized_point = {
    ...
    "units": mapping.units,  # ← Still showing original value "Wh"!
}
```
...
2. Works but gets overwritten/lost before being used in `normalized_point`
...
if mapping.device_class == "energy" and point_value is not None:
    if isinstance(point_value, (int, float)) and point_value != 0:
        ...
        point_value = point_value / 1000  # Wh → kWh ✓
        unit_to_use = "kWh"  # ← Use local variable, not mapping attribute
        _LOGGER.debug(
...
normalized_point = {
    ...
    "units": unit_to_use,  # ← Use local variable, always correct!
...
2. Defaults to `mapping.units` for non-energy sensors
3. Gets set to `"kWh"` for all energy sensors after conversion
4. `normalized_point["units"] = unit_to_use` always uses the correct value
5. No reliance on dataclass attribute modification

**Result**: Energy sensors now correctly display in kWh:
- Energia - Anno Scorso: `7.262 kWh` ✓
- Energia - Oggi: `8.606 kWh` ✓
- Energia - Ultimi 7 Giorni: `19.896 kWh` ✓
- Energia - Ultimi 30 Giorni: `266.266 kWh` ✓
...
**Version Files**:
2. `custom_components/.../const.py` (version 1.1.10)
3. `custom_components/.../manifest.json` (version 1.1.10)
...
**Lesson Learned**: When converting values at runtime, use local variables to track the converted state rather than trying to modify the source data structure. This is especially important with dataclasses which may be frozen or immutable.
...
**Verified Fixes**:
- ✅ DayWH, WeekWH, MonthWH, YearWH display in kWh (tested by user)
- ✅ Fresh installation shows kWh (not Wh)
- ✅ No entity registry cache issues
- ✅ All 258 sensors working correctly

**Energy Sensor Verification** (from user test):
```
Before v1.1.10:
  Energia - Oggi: 8.606 Wh ✗
