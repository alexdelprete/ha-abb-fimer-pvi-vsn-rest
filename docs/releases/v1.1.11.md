...
This release adds comprehensive attribute validation to ensure mapping file data is correct for Home Assistant, and
fixes bugs in the standalone test script.

**Key Improvements**:

- ✅ Validates device_class and unit consistency using official HA standards
- ✅ Handles exceptions for units without valid HA device_class (MOhm, kVAh)
- ✅ Fixes insulation resistance icon display (mdi:omega now shows correctly)
- ✅ Fixes vsn_client.py mapping URL and energy conversion

______________________________________________________________________

## Features Added

### 🔍 Attribute Validation in sensor.py
...
- ✅ Catches mapping file bugs at runtime with detailed error logs
- ✅ Provides defensive fixes when validation fails
- ✅ Respects mapping file as source of truth
- ✅ Validates data is correct for Home Assistant

**Example Error Log**:

```text
ERROR: Invalid unit 'Wh' for device_class 'energy' on sensor day_wh (device: 076543-3F71-2345).
...
- ✅ Insulation resistance sensors display `mdi:omega` icon correctly
- ✅ No device_class (correct - HA doesn't support MOhm)
- ✅ Unit: MOhm preserved

______________________________________________________________________

### 🐛 Bug #2: vsn_client.py Mapping URL Wrong
...
### 🐛 Bug #3: vsn_client.py Missing Energy Conversion
