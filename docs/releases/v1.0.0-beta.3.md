...
1. **OptionsFlow deprecation** warning (HA 2025.12)
1. **Sensor AttributeError** during initialization
...
**Problem:** Home Assistant detected blocking file operations in the event loop:
...
**Solution:** Refactored to use deferred async loading pattern following Home Assistant best practices:
...
**Problem:** Home Assistant logged deprecation warning:
...
**Problem:** Integration crashed during sensor setup:
...
1. Click "Update" to v1.0.0-beta.3
...
1. Verify sensors are loading correctly
1. Verify data updates are working
