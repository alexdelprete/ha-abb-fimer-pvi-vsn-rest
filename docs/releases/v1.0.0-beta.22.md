...
This release implements major improvements to entity identification and display naming to provide better namespace isolation and enable superior entity grouping in Home Assistant's UI.

## Highlights

### 🔧 Entity ID Format Update with Domain Prefix
...
**Benefits**:
- ✅ Namespace isolation prevents conflicts with other integrations
- ✅ Clear ownership indication
- ✅ Domain dynamically sourced from manifest.json (not hardcoded)
...
**Benefits**:
- ✅ Consistent format between unique_id and entity_id
- ✅ Domain dynamically sourced from manifest.json
- ✅ Proper namespace isolation

### 📊 Display Name Standardization (187 Entities)
...
|--------|-------|-------------|
| AC Power | Power AC | TYPE-first enables grouping |
| DC Voltage #1 | Voltage DC - String 1 | Consistent AC/DC format |
| Inverter AC energy produced - Lifetime | Energy AC - Produced Lifetime | Shorter, clearer |
| House consumption from inverter - Last 7 Days | Energy - House from Inverter Last 7 Days | Concise |
| Cabinet Temperature | Temperature - Cabinet | TYPE-first pattern |
| Grid Frequency | Frequency AC - Grid | AC designation added |
...
**Key Benefits**:
- ✅ TYPE-first naming enables perfect grouping in HA UI (all "Energy" entities group together, all "Power" entities group together, etc.)
- ✅ Consistent time period terminology (Lifetime, Since Restart, Last 7 Days, Last 30 Days, Last Year, Today)
- ✅ Clear AC/DC designation where relevant
- ✅ Shorter, more readable names
- ✅ Better hierarchy for related measurements
...
⚠️ **IMPORTANT**: This release contains breaking changes for existing beta users.
...
2. **Clarity**: Makes it immediately obvious which integration owns each entity
3. **Consistency**: Aligns unique_id and entity_id formats
4. **Future-Proofing**: Uses domain from manifest.json instead of hardcoded values
...
1. **Delete the integration** in Home Assistant (Settings → Devices & Services → ABB/FIMER PVI VSN REST → 3-dot menu → Delete)

2. **Stop Home Assistant**

3. **Edit `.storage/core.entity_registry`**:
...
6. **Verify** the new entity ID format:
...
Home Assistant persists entity IDs in the registry and restores them when an integration is reloaded, even if the unique_id changes. Manual cleanup ensures a clean slate with the new format.
...
If you're upgrading from versions before beta.20, you may also have serial number truncation issues. The entity registry cleanup described above will resolve both the domain prefix changes and any legacy truncation issues.
