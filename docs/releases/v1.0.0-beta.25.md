...
This release **properly** fixes the entity ID generation issue by implementing the correct approach: using
`has_entity_name=True` with technical device names. Beta.24's fix was
incorrect - it set `has_entity_name=False`, but Home Assistant completely ignores `suggested_object_id` in that mode.

______________________________________________________________________

## Critical Bug Fix

### 🐛 Entity ID Generation - The REAL Fix
...
1. Use technical device names: `abb_fimer_pvi_vsn_rest_datalogger_1100333a161234`
1. Set `suggested_object_id=point_name` (just the measurement name)
1. Home Assistant combines them into predictable entity IDs

**Result**:

```text
Entity ID: sensor.abb_fimer_pvi_vsn_rest_datalogger_1100333a161234_wlan0_essid ✅
Entity Name (on device page): "WiFi SSID" ✅
Device Name: "abb_fimer_pvi_vsn_rest_datalogger_1100333a161234" (technical but acceptable)
Friendly Name: "abb_fimer_pvi_vsn_rest_datalogger_1100333a161234 WiFi SSID"
```text
...
# Result: "abb_fimer_pvi_vsn_rest_datalogger_1100333a161234"
```text
...
- device_name: "abb_fimer_pvi_vsn_rest_datalogger_1100333a161234"
- suggested_object_id: "wlan0_essid"
- Result: `sensor.abb_fimer_pvi_vsn_rest_datalogger_1100333a161234_wlan0_essid` ✅

**Entity Names on Device Page**:

- Displayed as: "WiFi SSID" ✅ (clean and friendly)

**Full Friendly Names**:

- Displayed as: "abb_fimer_pvi_vsn_rest_datalogger_1100333a161234 WiFi SSID"
- (Technical prefix but includes entity name)

**Device Names**:

- Device list shows: "abb_fimer_pvi_vsn_rest_datalogger_1100333a161234"
...
1. **Restart Home Assistant** (recommended):
...
```text
sensor.abb_fimer_datalogger_vsn300_110033_3a16_1234_wifi_ssid
```text

**Beta.25 (correct)**:

```text
sensor.abb_fimer_pvi_vsn_rest_datalogger_1100333a161234_wlan0_essid
```text

**Differences**:

- ✅ Domain prefix added: `abb_fimer_pvi_vsn_rest_`
- ✅ Serial compacted: `110033_3a16_1234` → `1100333a161234`
- ✅ Point name from mapping: `wifi_ssid` → `wlan0_essid`
- ✅ No device model in entity ID
...
- `suggested_object_id = point_name` ✅
...
- ✅ Perfect entity IDs matching your requirements
- ✅ Clean entity names on device pages ("WiFi SSID", "AC Power")
- ✅ Predictable and consistent naming
- ✅ Namespace isolation with domain prefix

**What You Accept**:

- ⚠️ Device names are technical in device list
- ⚠️ Full friendly names include technical device prefix
...
1. `unique_id` remains stable (most important for entity identity)
1. Beta phase is for fixing exactly these kinds of issues
1. After beta → v1.0.0, entity IDs will be frozen
...
1. **Beta.24**: Still wrong (`has_entity_name=False` with `suggested_object_id` being ignored)
1. **Beta.25**: Correct (`has_entity_name=True` with technical device names) ✅
