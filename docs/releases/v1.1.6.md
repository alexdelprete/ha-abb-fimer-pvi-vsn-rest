...
This patch release refactors the codebase to move protocol-level constants from the integration layer to the client
library. This improves code organization, enables DRY (Don't
...
**Problem**: Protocol-level constants (Aurora state mappings, REST API endpoints, epoch offsets) were defined in the
integration layer (`const.py`), violating separation of
concerns and preventing the client library from being self-contained.

**Solution**: Move protocol knowledge to the client library where it belongs, maintaining backward compatibility through
re-exports.
