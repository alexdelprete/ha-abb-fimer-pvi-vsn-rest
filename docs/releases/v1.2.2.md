...
This release completes the fix for Issue #25 that was only partially fixed in v1.2.1. Users with VSN devices that have
authentication disabled can now fully use the integration.
...
**Problem in v1.2.1**: While v1.2.1 fixed the device detection to handle HTTP 200 responses (no authentication
required), subsequent data fetching still failed. The error message
was:
