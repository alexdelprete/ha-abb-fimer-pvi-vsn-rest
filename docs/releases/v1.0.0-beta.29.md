...
1. **sys_time sensor** - Showing "32 seconds ago" instead of actual date/time (beta.28 fix caused relative time display)

Both issues required deeper investigation to identify the true root causes. Beta.28's approaches were partially correct
...
### 🐛 device_type - Still "unknown" After Beta.28
...
Beta.28 assumed the discovered_devices list contained datalogger entries with `device_type="datalogger"`, but this
wasn't true!
...
Add one line in
[discovery.py:362](d:%5COSILifeDrive%5CDev%5CHASS%5Cha-abb-fimer-pvi-vsn-rest%5Ccustom_components%5Cabb_fimer_pvi_vsn_rest%5Cabb_fimer_vsn_rest_client%5Cdiscovery.py#L362):
...
- ✅ Datalogger from livedata correctly gets device_type="datalogger"
- ✅ Beta.28's injection logic now injects the correct value
- ✅ Normalized data has correct device_type
- ✅ Sensor attributes show `device_type: "datalogger"`

______________________________________________________________________

### 🕐 sys_time - Shows "32 seconds ago" Instead of Date
...
Home Assistant's timestamp device_class is **DESIGNED** to display relative time ("X ago", "X minutes ago", etc.). This is intentional behavior that cannot be overridden while
using the timestamp device_class.

**The Catch-22**:

- Need `device_class: "timestamp"` → Conversion runs → datetime object created → HA forces relative time display
- Remove `device_class: "timestamp"` → Conversion doesn't run → Raw integer displayed
...
- **Beta 28**: Added it back to enable conversion (Aurora epoch → datetime)
...
1. **Remove device_class** from mapping file
...
1. **Modify conversion logic** to check point_name instead of device_class
   ([sensor.py:415-433](d:%5COSILifeDrive%5CDev%5CHASS%5Cha-abb-fimer-pvi-vsn-rest%5Ccustom_components%5Cabb_fimer_pvi_vsn_rest%5Csensor.py#L415-L433)):
...
- ✅ Conversion runs based on point_name check (not device_class)
- ✅ Returns formatted string: "2025-11-13 10:15:32"
- ✅ No device_class → HA displays as-is (not relative time)
- ✅ User sees actual date/time string
...
| File | Lines | Change | |------|-------|--------| | discovery.py | 362 | Add device_type assignment for datalogger | |
sensor.py | 415-433 | Modify conversion to check point_name
instead of device_class | | sensor.py | 425 | Return formatted string instead of datetime object | |
vsn-sunspec-point-mapping.json | 544 | Remove device_class timestamp | |
manifest.json | 13 | Version bump to 1.0.0-beta.29 | | const.py | 4 | Version bump to 1.0.0-beta.29 |
...
```text
Entity: sensor.abb_datalogger_vsn300_110033_3a16_1234_firmware_version
Attributes:
  device_type: unknown
...
```text
Entity ID: sensor.power_one_inverter_pvi_10_0_outd_076543_3f71_2345_sys_time
...
**Beta.28's Logic**: Inject device_type from discovered_devices before normalization **Assumption**: discovered_devices has correct device_type **Reality**: discovered_devices from
livedata had device_type="unknown" **Missing**: Discovery.py never set device_type for datalogger entries from livedata

**Beta.29 Fix**: Set device_type in discovery at the source, making beta.28's injection actually work.

### Issue 2: sys_time

**Beta.28's Logic**: Add device_class: "timestamp" to trigger conversion **Assumption**: Timestamp device_class shows formatted date/time **Reality**: Timestamp device_class forces
relative time display ("X ago") **Incompatibility**: HA's design doesn't allow both timestamp device_class and formatted display
