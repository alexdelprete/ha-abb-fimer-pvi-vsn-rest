...
This release fixes multiple entity display issues including diagnostic icons, state entity mappings, timestamp display
format, unit validation errors, and entity naming
improvements.

## Highlights

### 🎨 Diagnostic Entity Icons Now Display

**Problem**: Diagnostic entities and VAh entities had icons defined in the mapping file but they weren't displaying in
Home Assistant.

**Root Cause**: The icon data flow was broken at three points - icons weren't being loaded from the mapping file, passed
through the normalizer, or applied to sensor entities.
...
- ✅ 3 VAh entities (E2_runtime, E2_7D, E2_30D) display lightning bolt icon (⚡ `mdi:lightning-bolt`)

**Files Changed**:

- [mapping_loader.py:43](../../custom_components/abb_fimer_pvi_vsn_rest/abb_fimer_vsn_rest_client/mapping_loader.py#L43)
  - Added icon field to dataclass
- [mapping_loader.py:188-189](../../custom_components/abb_fimer_pvi_vsn_rest/abb_fimer_vsn_rest_client/mapping_loader.py#L188-L189)
  - Load icon from JSON
- [normalizer.py:202](../../custom_components/abb_fimer_pvi_vsn_rest/abb_fimer_vsn_rest_client/normalizer.py#L202) -
  Pass icon to normalized data
- [sensor.py:324-328](../../custom_components/abb_fimer_pvi_vsn_rest/sensor.py#L324-L328) - Apply icon to entity

### 🔧 State Entity Mappings Fixed

**Problem**: Four Aurora protocol state entities were showing numeric codes (e.g., "6", "2") instead of descriptive text
(e.g., "Run", "MPPT").

**Root Cause**: Dictionary key mismatch in `STATE_ENTITY_MAPPINGS`. The keys used old naming ("GlobState", "DC1State")
but actual SunSpec normalized names were different
("GlobalSt", "DcSt1").
...
STATE_ENTITY_MAPPINGS = {
    "GlobState": GLOBAL_STATE_MAP,    # ❌ Mismatch
    "DC1State": DCDC_STATE_MAP,       # ❌ Mismatch
    "DC2State": DCDC_STATE_MAP,       # ❌ Mismatch
    "InvState": INVERTER_STATE_MAP,   # ❌ Mismatch
    ...
}
```text
...
```python
STATE_ENTITY_MAPPINGS = {
    "GlobalSt": GLOBAL_STATE_MAP,      # ✅ Correct
    "DcSt1": DCDC_STATE_MAP,           # ✅ Correct
    "DcSt2": DCDC_STATE_MAP,           # ✅ Correct
    "InverterSt": INVERTER_STATE_MAP,  # ✅ Correct
    ...
}
```text
...
- ✅ `dc_st1` now shows "MPPT" instead of 2
- ✅ `dc_st2` now shows "MPPT" instead of 2
...
### 🕐 System Time Display Format Fixed
...
- ✅ `sys_time` now displays as "2025-11-05 12:04:30" instead of "1 minute ago"
- ✅ Still shows the inverter's system clock accurately
...
- ✅ No more validation errors in Home Assistant logs
- ✅ `wlan0_link_quality` correctly displays as percentage without inappropriate device_class
...
### 📝 Removed Redundant "Device" Prefix from M1 Entities
...
- ✅ Cleaner, more concise entity names in Diagnostic section
- ✅ Better readability in device info cards
- ✅ Consistent with Home Assistant UI conventions
...
vsn-sunspec-point-mapping.json
  → PointMapping.icon (mapping_loader.py)
  → normalized_point["icon"] (normalizer.py)
  → sensor._attr_icon (sensor.py)
  → Home Assistant entity icon
```text
...
1. **Reload the integration** in Home Assistant (Settings → Devices & Services → ABB/FIMER PVI VSN REST → 3-dot menu → Reload)
...
1. **Verify the fixes**:
   ...
   - ✅ Diagnostic entities show information icon (ℹ️)
   - ✅ State entities show text ("Run", "MPPT") instead of numbers
   - ✅ System time shows formatted date/time instead of "X ago"
   - ✅ No validation errors in logs
   - ✅ M1 entities have clean names without "Device" prefix
