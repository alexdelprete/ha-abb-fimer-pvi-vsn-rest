...
This release fixes multiple entity display issues including diagnostic icons, state entity mappings, timestamp display format, unit validation errors, and entity naming improvements.
...
2. **normalizer.py**: Passed icon through to normalized point data
3. **sensor.py**: Applied icon to entity via `self._attr_icon`
...
**Root Cause**: Dictionary key mismatch in `STATE_ENTITY_MAPPINGS`. The keys used old naming ("GlobState", "DC1State") but actual SunSpec normalized names were different ("GlobalSt", "DcSt1").
...
**Note**: The data_size validation errors mentioned by the user were actually from cached old entities with truncated serial numbers (from before beta.20). These will disappear after deleting the old entities or removing/re-adding the integration.
...
2. Ran `python generate_mapping.py` to create Excel
3. Ran `python convert_to_json.py` to create JSON
4. JSON automatically copied to integration data folder
...
2. **For users upgrading from pre-beta.20 versions**: If you still see validation errors about "bytes" units, you have orphaned entities with old truncated serial numbers. Either:
