...
This release fixes a critical bug introduced in beta.13 where `has_entity_name` was incorrectly changed from `False` to
`True`, breaking the `suggested_object_id` mechanism that
...
```text
sensor.power_one_inverter_pvi_10_0_outd_076543_3f71_2345_ac_power
sensor.abb_fimer_datalogger_vsn300_110033_3a16_1234_available_ram
```text
...
- ✅ Entity IDs now use the correct format: `sensor.abb_fimer_pvi_vsn_rest_{device_type}_{serial_compact}_{point_name}`
- ✅ Friendly display names remain unchanged: "System Load Average", "AC Power", etc.
- ✅ Device names remain friendly: "ABB Datalogger VSN300 (110033-3A16-1234)"
- ✅ Unique IDs and entity IDs now have consistent format
...
- Device name: Friendly format (e.g., "ABB Datalogger VSN300 (110033-3A16-1234)")
...
```text
sensor.abb_fimer_datalogger_vsn300_110033_3a16_1234_system_load_average
```text

**After (beta.24 - FIXED)**:

```text
sensor.abb_fimer_pvi_vsn_rest_datalogger_1100333a161234_system_load_average
```text
...
We sincerely apologize for this back-and-forth with entity ID changes. The bug was introduced in beta.13 and went
undetected until users reported the issue. This fix restores the
