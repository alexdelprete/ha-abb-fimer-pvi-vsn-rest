...
This release fixes a critical bug introduced in beta.13 where `has_entity_name` was incorrectly changed from `False` to `True`, breaking the `suggested_object_id` mechanism that controls entity ID generation.
...
- ✅ Device names remain friendly: "ABB Datalogger VSN300 (111033-3N16-1421)"
...
2. `[hash]` - chore(release): bump version to v1.0.0-beta.24
...
2. **Clean entity registry** (CRITICAL):
   ```bash
   # Stop Home Assistant
...
We sincerely apologize for this back-and-forth with entity ID changes. The bug was introduced in beta.13 and went undetected until users reported the issue. This fix restores the correct behavior from beta.12.
