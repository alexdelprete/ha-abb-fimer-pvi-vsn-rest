...
**Root Cause**: The `AbortFlow` exception raised by `_abort_if_unique_id_configured()` was being caught by the broad `except Exception` handler in the config flow, which converted it to an "unknown" error.
...
**For existing users**: Your current configuration will continue to work. The new notification options will use their default values (repair notifications enabled, startup notifications disabled).
