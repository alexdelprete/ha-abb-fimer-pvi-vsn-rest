...
This release brings comprehensive entity improvements with proper device classes, icons, and Aurora protocol integration
for better data accuracy and user experience.

## Highlights

### 📊 Device Class Mapping Improvements
...
- Fixed Isolation_Ohm1 unit from Ω to MΩ (verified from feeds data)
...
- ⚡ **Energy icon** (`mdi:lightning-bolt`) for 3 VAh apparent energy entities
  - E2_runtime, E2_7D, E2_30D
- ℹ️ **Diagnostic icon** (`mdi:information-box-outline`) for all 51 diagnostic entities
...
| Entity | Description | States | Example Values | |--------|-------------|--------|----------------| | **GlobState** | Global operational state | 42 | "Run", "Wait Sun / Grid",
"Ground Fault" | | **DC1State** | DC string 1 state | 20 | "MPPT", "Ramp Start", "DcDc OFF" | | **DC2State** | DC string 2 state | 20 | "MPPT", "Ramp Start", "DcDc OFF" | |
**InvState** | Inverter state | 38 | "Run", "Stand By", "Checking Grid" | | **AlarmState** | Alarm (VSN700) | 65 | "No Alarm", "Ground Fault", "Grid Fail" | | **AlarmSt** | Alarm
(VSN300) | 65 | "No Alarm", "Ground Fault", "Grid Fail" |
...
- ✅ 258 total entities mapped
- ✅ 135 entities with device_class (measurement entities)
- ✅ 51 diagnostic entities (with information icons)
- ✅ 3 VAh entities (with energy icons)
- ✅ 6 status entities (with human-readable states)
...
- Special measurement types (MΩ, RPM, channels, cycles) without standard HA device classes
