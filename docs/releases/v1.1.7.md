...
This release transforms the sensor mapping workflow by codifying ALL sensor fixes from v1.1.3-v1.1.7 into the generator
script. The generator is now the **single source of truth**
for sensor metadata, eliminating manual JSON/Excel editing and preventing future mapping inconsistencies.

**Key Achievement**: Precision field support added to generator, ensuring all 258 sensors have consistent decimal
precision rules.

**User Impact**: No breaking changes for existing installations. All sensor attributes remain the same, but future
mapping updates will be more reliable and consistent.

______________________________________________________________________

## What's Changed

### 🏗️ Generator Script: Comprehensive Enhancement
...
**Solution**: Extended the generator script (`generate_mapping.py`) to include ALL sensor metadata, making it the
...
- State sensors → `""` (empty string) - Display text, not numbers
- Power/Energy counters → `0` decimals (W, kW, Wh, kWh, VAh, s, B)
- Voltage/Temperature → `1` decimal (V, mA, %, °C, °F, MB, GB, Ah)
- Current/Frequency → `2` decimals (A, Hz, MOhm, MΩ, kΩ)
- Small resistance/Cycles → `0` decimals (Ω, Ohm, cycles, channels)
- Special case: `sys_load` → `2` decimals (float 0.00-100.00)
...
EXCEL_HEADERS = [
    *MODEL_FLAGS,
    ...
    # ... other columns ...
    "Entity Category",
    "Suggested Display Precision",  # ← NEW in v1.1.7
    "Available in Modbus",
    "Data Source",
    "Model_Notes",
]
...
Documented Wh→kWh conversion as defensive code:
...
- **Correct Workflow**: Edit generator → Regenerate → Test → Commit
...
- ❌ NEVER edit JSON mapping files directly
- ❌ NEVER edit Excel mapping files directly
- ❌ NEVER add sensor attributes to runtime code
...
1. **Unit-based auto-calculation** (via `_get_suggested_precision()`)

   - Power/Energy: W, kW, Wh, kWh → `0`
   - Voltage/Temperature: V, °C, % → `1`
   - Current/Frequency: A, Hz, MOhm → `2`

1. **Device class fallback** (for sensors without units)

   - power, reactive_power, timestamp → `0`
   - voltage, temperature → `1`
   - current, frequency, energy → `2`

1. **Default**: `0` (for unitless numeric sensors)
...
| Version | Columns | Precision Field | Notes | |---------|---------|-----------------|-------| | v1.1.2 | 26 | No | Manual Excel editing | | v1.1.3 | 27 | Yes (manual) | Added
manually to JSON | | v1.1.7 | 28 | Yes (generated) | Generator creates precision field |
...
- **ALWAYS** use generator script to modify sensor metadata:
  ...
  1. Add/modify entry in `SUNSPEC_TO_HA_METADATA` dictionary
  1. Run `python scripts/vsn-mapping-generator/generate_mapping.py`
  1. Run `python scripts/vsn-mapping-generator/convert_to_json.py`
  1. Test changes
  1. Commit generator + all generated files

See [CLAUDE.md](../../CLAUDE.md) section "⚠️ MANDATORY: Sensor Mapping Workflow" for complete instructions.
...
- ✅ All 258 sensors have precision field
- ✅ 6 state sensors have `precision=""` (empty string)
- ✅ 3 cycle counters have `state_class="total_increasing"`, `precision=0`
- ✅ Power sensor (W): `precision=0`
- ✅ Voltage sensors (V): `precision=1`
- ✅ Current sensors (A): `precision=2`
- ✅ Riso sensor: `unit="MOhm"`, `icon="mdi:omega"`, `precision=2`
- ✅ Energy sensors: kWh → `precision=0`, Wh with decimals → `precision=2`
- ✅ Integer count sensors: `precision=0`
...
✓ Excel file created: vsn-sunspec-point-mapping.xlsx
  Total rows: 258
  Sheets: All Points, Summary, Changelog

✓ JSON file created: vsn-sunspec-point-mapping.json
  Total entries: 258
