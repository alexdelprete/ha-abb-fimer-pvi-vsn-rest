...
This release focuses on cleanup and consistency by removing duplicate columns from the mapping file and standardizing all references to use HA-prefixed column names. This change
has **zero functional impact** but improves code clarity and maintainability.
...
**Rationale**: These columns represent Home Assistant concepts (`SensorDeviceClass`, `SensorStateClass`, `unit_of_measurement`). Using the "HA" prefix makes this explicit and
removes confusion about whether SunSpec values needed transformation to HA values (they don't—they're identical).
...
1. **Reduced Confusion**: No question about whether transformation is needed
1. **Smaller Files**: JSON files ~10% smaller without duplicate keys
1. **Better Maintenance**: Single source of truth for each value
1. **Code Clarity**: Consistent naming throughout codebase
