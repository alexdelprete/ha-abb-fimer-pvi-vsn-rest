...
This release focuses on developer tooling improvements with no changes to integration functionality. All changes are in the mapping generator scripts, improving code quality,
maintainability, and compliance with Python best practices.
...
#### 🔧 Linting Compliance (All 22 Errors Fixed)
...
- Cyclomatic complexity: 71 (limit is 25) ❌
- All logic in one massive function

**After:**

- Main function: 74 lines (86% reduction!)
- Cyclomatic complexity: Well under 25 ✅
- 10 focused helper functions

**Helper Functions Created:**

1. `_load_data_sources()` - Load SunSpec metadata, M64061, feeds, status data
1. `_extract_point_sets()` - Extract VSN300/VSN700 point sets from JSON files
1. `_process_sunspec_points()` - Process standard SunSpec points
1. `_process_m64061_points()` - Process M64061 proprietary points
1. `_process_abb_proprietary_points()` - Process ABB proprietary points
1. `_process_vsn300_only_points()` - Process VSN300-only points
1. `_process_vsn700_only_points()` - Process VSN700-only points
1. `_process_status_points()` - Process /status endpoint points
1. `_get_cell_value_for_header()` - Map Excel headers to row data values
1. `_create_excel_workbook()` - Create and format Excel workbook
...
- ✅ Code Quality - All ruff checks pass (100% compliance)
...
1. `0fe5ede` - refactor(scripts): reduce complexity of generate_mapping_excel_complete()
1. `4f407b4` - chore(mapping): regenerate VSN-SunSpec point mapping files
...
1. **Code Quality**: Refactored code is easier to understand and maintain
1. **Testing**: Regenerated mapping files verify the refactoring works correctly
