...
This release adds support for single-phase (M101) and split-phase (M102) inverters, and improves energy sensor display
names for better clarity. Previously, only three-phase (M103)
...
Special thanks to **@SidShetye** for reporting [Issue
#14](https://github.com/alexdelprete/ha-abb-fimer-pvi-vsn-rest/issues/14) and providing test data from their
PVI-4.2-OUTD
...
**Problem**: Users with single-phase inverters (M101) like the PVI-4.2-OUTD were experiencing missing sensors. The
integration only mapped three-phase (M103) point names, but
...
**Solution**: Runtime pattern normalization in the data normalizer. SunSpec defines M101 (single-phase), M102
(split-phase), and M103 (three-phase) with **identical point
