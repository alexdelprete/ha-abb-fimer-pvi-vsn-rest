...
This release fixes duplicate sensor entries by implementing VSN name normalization that prioritizes VSN300 (SunSpec-based) names as canonical when merging sensors that exist in both VSN300 and VSN700 models.
...
3. `custom_components/.../sensor.py` (1 change):
...
5. `docs/vsn-sunspec-point-mapping.json`
6. `docs/vsn-sunspec-point-mapping.xlsx`
7. `scripts/vsn-mapping-generator/output/vsn-sunspec-point-mapping.json`
8. `scripts/vsn-mapping-generator/output/vsn-sunspec-point-mapping.xlsx`

**Version Files**:

9. `custom_components/.../const.py` (version 1.1.12)
10. `custom_components/.../manifest.json` (version 1.1.12)
...
2. VSN700 "Riso" becomes "Isolation_Ohm1" for deduplication key
3. Deduplication merges rows with same SunSpec normalized name
4. Merged entry has both VSN300 and VSN700 REST names
5. Runtime uses appropriate REST name based on VSN model detected
